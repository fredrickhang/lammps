...
#include "pair_sph_taitwater_temperature.h"
#include <cmath>
#include "atom.h"
#include "force.h"
#include "comm.h"
...
PairSPHTaitwaterTemperature::PairSPHTaitwaterTemperature(LAMMPS *lmp) : Pair(lmp)
{
  restartinfo = 0;
  first = 1;
  single_enable = 0;
...
PairSPHTaitwaterTemperature::~PairSPHTaitwaterTemperature() {
  if (allocated) {
...
void PairSPHTaitwaterTemperature::compute(int eflag, int vflag) {
...
void PairSPHTaitwaterTemperature::allocate() {
  allocated = 1;
  int n = atom->ntypes;
...
void PairSPHTaitwaterTemperature::settings(int narg, char **/*arg*/) {
  if (narg != 0)
    error->all(FLERR,
        "Illegal number of arguments for pair_style sph/taitwater/temperature");
}
...
void PairSPHTaitwaterTemperature::coeff(int narg, char **arg) {
  if (narg != 6)
    error->all(FLERR,
        "Incorrect args for pair_style sph/taitwater/temperature coefficients");
  if (!allocated)
    allocate();
...
double PairSPHTaitwaterTemperature::init_one(int i, int j) {

  if (setflag[i][j] == 0) {
    error->all(FLERR,"All pair sph/taitwater/temperature coeffs are not set");
  }
